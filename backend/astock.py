...
_TDX_SERVERS = [
    # TCP 可连不代表行情可用；这些节点会用贵州茅台日 K 实际探测后再选用。
    ("183.60.224.178", 7709),
    ("183.60.224.177", 7709),
    ("119.29.19.242", 7709),
    ("117.34.114.18", 7709),
...
_tdx_server_cache: list[tuple[str, int] | None] = [None]
_tdx_server_lock = threading.Lock()


def _new_mootdx_client(server: tuple[str, int]):
    from mootdx.quotes import Quotes

    return Quotes.factory(
        market="std", server=server, timeout=2, auto_retry=False,
    )


def _mootdx_client():
    """创建可实际返回行情的 mootdx 客户端，并缓存已验证的服务器。

    mootdx 0.11.x 在 BESTIP 为空时会直接报错；部分 7709 节点虽然 TCP
    可连接，却返回空行情。因此不能只做端口探测，必须用一条真实日 K 验证。
    """
    try:
        from mootdx.quotes import Quotes  # noqa: F401 — 惰性依赖探测
    except ImportError as e:
        raise DependencyMissing("mootdx 未安装：pip install mootdx") from e

    cached = _tdx_server_cache[0]
    if cached:
        return _new_mootdx_client(cached)
    ...
    with _tdx_server_lock:
        ...
        for server in _TDX_SERVERS:
            try:
                client = _new_mootdx_client(server)
                probe = client.bars(symbol="600519", frequency=9, offset=2)
                if probe is not None and not probe.empty:
                    _tdx_server_cache[0] = server
                    return client
                errors.append(f"{server[0]}: 空行情")
            except Exception as e:  # noqa: BLE001 — 逐节点降级
                errors.append(f"{server[0]}: {e}")
