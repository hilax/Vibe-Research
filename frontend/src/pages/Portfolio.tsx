...
const pnlColor = (v: number) => (v > 0 ? "text-market-up" : v < 0 ? "text-market-down" : "text-muted-foreground");
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div>
            ...
            <input value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="6 位代码"
              className="w-28 rounded-lg border border-border bg-surface-2/55 dark:bg-black/20 px-3 py-2 text-sm outline-none focus:border-primary/50" />
          </div>
          <div>
              ...
              <tbody>
                {holdings.map((h) => (
                  <tr key={h.code}
                    ...
                    <td className="px-2 py-2.5">
                      <button onClick={(e) => { e.stopPropagation(); remove(h.code); }}
        ...
        <div className="flex flex-wrap items-end gap-2">
          <div>
            ...
            <input value={cCode} onChange={(e) => setCCode(e.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="6 位代码"
              className="w-24 rounded-lg border border-border bg-surface-2/55 dark:bg-black/20 px-3 py-2 text-sm outline-none focus:border-primary/50" />
          </div>
          <div>
            <label className="mb-1 block text-xs text-muted-foreground">清仓日期</label>
            <input type="date" value={cDate} onChange={(e) => setCDate(e.target.value)}
              className="rounded-lg border border-border bg-surface-2/55 dark:bg-black/20 px-3 py-2 text-sm outline-none focus:border-primary/50" />
          </div>
          <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {closed.map((c, i) => (
                  <tr key={i}
                    onClick={() => navigate(`/stock-kline/${c.code}`)}
                    className="cursor-pointer border-b border-border/30 transition-colors hover:bg-primary/5">
                    <td className="px-2 py-2.5">
                      <Link to={`/stock-kline/${c.code}`} onClick={(e) => e.stopPropagation()}
                        className="font-medium hover:text-primary">
                        {c.name}
                      </Link>
                    ...
                    <td className="px-2 py-2.5">
                      <button onClick={(e) => { e.stopPropagation(); removeClosed(i); }}
                        className="text-muted-foreground/50 hover:text-destructive" title="删除">
                        <Trash2 className="h-3.5 w-3.5" />
                      </button>
                    </td>
